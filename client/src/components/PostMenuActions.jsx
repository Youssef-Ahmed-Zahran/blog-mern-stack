...
const PostMenuActions = ({ post }) => {
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      return makeRequest.patch("/users/save", {
        postId: post._id,
      });
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      toast.error(error.response?.message || "Failed to save/unsave post");
    },
  });
  ...
  const featureMutation = useMutation({
    mutationFn: () => {
      return makeRequest.patch("/posts/feature", {
        postId: post._id,
      });
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      toast.error(error.response?.message || "Failed to feature this post");
    },
  });
  ...
  return (
    <div className="">
      ...
      {currentUser && (post.user?._id === currentUser._id || isAdmin) && (
        <div
          className="flex items-center gap-2 py-2 text-sm cursor-pointer"
